...
1. understand Creative's *EaxMan.dll* so it can be used in Unreal Tournament v469+
...
Here is a demonstration how it sounds with the patch applied: [Unreal Tournament's Special EAX Version: Deck16 Demo - YouTube](https://www.youtube.com/watch?v=oTwO91NBOn0)
...
The shortcomings of the solution presented above is its limited scope. The two main limitations are:
...
EAGLE™ can be used to confirm and understand the information read from *.eal* files. But it's greatest potential lies in the **Import Plugin-In SDK** which might be a way to understand the geometry data stored inside *.eal* files.
...
There are example tools coming with the *EaxManager SDK* which are more complete and useful for a proper implementation.
...
All static data that can be read out with EAXManager API calls are retrieved. The according API calls are:
...
An example out looks like this:
...
According to the documentation, *EAXManager* uses the same coordinate system as
*DirectSound* with positive values of X, Y and Z pointing forward, right and up. It is unknown to the author if Unreal Tournament uses the same orientation, although the v469 patch programmers can easily check the source code.
...
What `EaxMan.dll` actually does is reading all EAX parameters from the *.eal* file and returns them. It does not set up *DirectSound3D* / *EAX* calls but only provides the parameters which the program then needs to use.
...
The *ReadEalFiles* tool provided in this repository does implement a way to read all the data from the *.eal* files. And it compares it to the results obtained with *EAXManager* to verify the results. The output of that tool when analysing *CTF-Gauntlet.eal* looks like this:
...
It uses the algorithm from *ReadEalFiles* to read the environment presets from a *.eal* file. It then creates a *EFX* call with the parameters from the preset and uses *OpenAL Soft* to play the sounds with the effects.
